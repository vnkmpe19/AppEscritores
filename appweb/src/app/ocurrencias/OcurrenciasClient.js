...
export default function OcurrenciasClient() {
  ...
  const inicializar = async () => {
    ...
    if (authUser) {
      ...
      setUser({ name: authUser.user_metadata?.nombre || authUser.email.split('@')[0], role: 'Escritor' });
  ...
  const saveNote = async (noteData) => {
    ...
    if (editingNote) {
      ...
      else if (data) setNotes([data, ...notes]);
    }
    closeEditor();
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#FFF5F5] font-sans text-slate-800 overflow-x-hidden">
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarExpanded ? 'md:ml-64' : 'md:ml-24'} ml-0 p-4 md:p-8 flex flex-col h-screen overflow-y-auto`}>
        ...
        <div className="max-w-6xl mx-auto relative min-h-[500px] w-full">
          {loading ? (
